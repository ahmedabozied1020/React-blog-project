...
const PostDetails = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      // If the post data was passed in the state, use it
      if (location.state && location.state.post) {
        setPost(location.state.post);
      } else {
        // Otherwise, fetch the post data
        try {
  ...
  return (
    <div className="w-4/5 mx-auto flex flex-col items-center mt-10 ">
            ...
            <div className="bg-green-400 rounded-xl h-12 w-1/4 flex items-center justify-center absolute left-1 top-[4px] group-hover:w-[184px] z-10 duration-500">
              <span className="relative flex-shrink-0 w-6 h-6 bg-white text-[#7808d0] rounded-full grid place-items-center overflow-hidden transition-all duration-300 group-hover:bg-black group-hover:text-white">
                <svg
                  ...
                  height="22px"
                  width="22px"
                  className="absolute transition-transform duration-300 transform group-hover:scale-75"
                >
                  <path
